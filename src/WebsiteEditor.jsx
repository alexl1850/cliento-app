...
export default function WebsiteEditor({ biz, liveUrl, onBack }) {
  ...
  const [cachedHtml, setCachedHtml]   = useState(""); // cache HTML so we don't need to re-fetch
    ...
    try {
      const res = await fetch("/api/edit-website", {
        ...
        body: JSON.stringify({
          ...
          currentUrl,
          currentHtml: cachedHtml, // pass cached HTML so server doesn't need to fetch
          biz,
        }),
      });
      ...
      setCurrentUrl(d.url);
      if (d.html) setCachedHtml(d.html); // cache the updated HTML for next edit
      setInstruction("");
      setPhase("done");
    } catch (e) {
