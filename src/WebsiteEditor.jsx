...
// Screenshot image component — never blocked unlike iframes
function SitePreviewImg({ url }) {
  const [errored, setErrored] = useState(false);
  const screenshotUrl = `https://image.thum.io/get/width/1200/crop/700/${url}`;
  if (errored) return (
    <div style={{height:"280px",background:"linear-gradient(135deg,#0D1117,#1E3A5F)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:"10px"}}>
      <div style={{fontSize:"2em"}}>🌐</div>
      <div style={{fontWeight:700,color:"#fff",fontSize:"0.9em"}}>Click to open your website</div>
      <div style={{fontSize:"0.75em",color:"rgba(255,255,255,0.4)",wordBreak:"break-all",maxWidth:"260px",textAlign:"center"}}>{url}</div>
    </div>
  );
  return (
    <img
...
export default function WebsiteEditor({ biz, liveUrl, initialHtml, onBack }) {
  const [instruction, setInstruction] = useState("");
  const [phase, setPhase]             = useState("idle");
  const [currentUrl, setCurrentUrl]   = useState(liveUrl || "");
  const [cachedHtml, setCachedHtml]   = useState(initialHtml || "");
    ...
    try {
      const res = await fetch("/api/edit-website", {
        ...
        body: JSON.stringify({
          ...
          currentUrl,
          currentHtml: cachedHtml, // always pass the cached HTML — never fetch from URL
          biz,
        }),
      });
      ...
      if (d.html) {
        ...
        // Save updated HTML back to localStorage so it persists across sessions
        localStorage.setItem(`site_html_${biz?.name}`, d.html);
      }
      setInstruction("");
      setPhase("done");
    } catch (e) {
    ...
    <div style={{ fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      {currentUrl && (
        <a
          ...
          style={{ display: "block", background: "#fff", border: "1.5px solid #E5E7EB", borderRadius: "12px", overflow: "hidden", marginBottom: "16px", textDecoration: "none", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", transition: "all 0.2s" }}
        ...
        >
          {/* Browser chrome */}
          <div style={{ background: "#F1F3F4", padding: "10px 14px", borderBottom: "1px solid #E5E7EB", display: "flex", alignItems: "center", gap: "8px" }}>
            <div style={{ display: "flex", gap: "5px" }}>
              {["#FF5F56", "#FFBD2E", "#27C93F"].map(c => <div key={c} style={{ width: "10px", height: "10px", borderRadius: "50%", background: c }} />)}
            </div>
            <div style={{ flex: 1, background: "#fff", borderRadius: "6px", padding: "4px 10px", fontSize: "0.72em", color: "#6B7280", border: "1px solid #E5E7EB", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontFamily: "monospace" }}>
              {currentUrl}
            </div>
            <span style={{ fontSize: "0.72em", color: "#2563EB", fontWeight: 700, flexShrink: 0 }}>Open ↗</span>
          </div>
          {/* Screenshot preview — uses free screenshot service, never blocked */}
          <SitePreviewImg url={currentUrl}/>
        </a>
      )}
      ...
      {currentUrl && (
        <div style={{display:"flex",gap:"8px",marginBottom:"16px",marginTop:"-8px"}}>
          <button onClick={copyUrl} style={{flex:1,padding:"10px",borderRadius:"9px",border:"1.5px solid #E5E7EB",background:"#fff",color:copied?"#16A34A":"#6B7280",fontWeight:600,fontSize:"0.84em",cursor:"pointer",fontFamily:"inherit",transition:"all 0.15s"}}>
            {copied?"✓ Copied!":"📋 Copy link"}
          </button>
          <a href={currentUrl} target="_blank" rel="noopener noreferrer" style={{flex:1,padding:"10px",borderRadius:"9px",background:"#2563EB",color:"#fff",fontWeight:700,fontSize:"0.84em",textAlign:"center",textDecoration:"none",display:"block"}}>
            🌐 Open website
          </a>
        </div>
      )}
