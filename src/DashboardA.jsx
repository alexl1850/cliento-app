  ...
  return (
    <div>
      ...
      {activeTool && activeTool!=="connect-shopify" && (
        <ToolPanel toolId={activeTool} biz={biz} industry={null} existing={results[activeTool]} profile={profile}
          onBack={()=>setActiveTool(null)}
          onSave={(id,content)=>{setResults(r=>({...r,[id]:content}));setActiveTool(null);}}
...
// TOOL PANEL
// ═════════════════════════════════════════════════════════════════════════════
function ToolPanel({toolId,biz,industry,existing,onBack,onSave,profile}) {
  ...
  if (toolId === "website") {
    return <WebsiteBuilderPanel biz={biz} profile={profile} onBack={onBack} onSave={onSave}/>;
  }
...
// ═════════════════════════════════════════════════════════════════════════════
function WebsiteBuilderPanel({ biz, profile, onBack, onSave }) {
  const [phase, setPhase] = useState("ready"); // ready | gaps | building | done | error
  const [liveUrl, setLiveUrl] = useState(profile?.live_url || "");
  const [errorMsg, setErrorMsg] = useState("");
  const [copied, setCopied] = useState(false);

  // Extra fields for gaps in the profile
  ...
  const missing = [];
  if (!biz.phone)       missing.push("phone");
  if (!biz.email)       missing.push("email");
  if (!biz.description) missing.push("description");
  if (!biz.suburb)      missing.push("suburb");

  const hasCritical = biz.name && biz.suburb && biz.description;
    ...
    try {
      const res = await fetch("/api/build-website", {
        ...
        body: JSON.stringify({
          intake: {
            biz_name:     biz.name,
            owner_name:   biz.owner || biz.name,
            base_suburb:  biz.suburb,
            service_area: biz.suburb + " and surrounds",
            phone:        biz.phone || "",
            email:        biz.email || "",
            years:        "Established local business",
            services:     biz.menu || biz.description,
            menu:         biz.menu || "",
            difference:   biz.description,
    ...
    <div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:"12px"}}>
          ...
          <div>
            ...
            <div style={{fontSize:"0.8em",color:"rgba(255,255,255,0.5)"}}>
              {liveUrl ? "Your website is live — view, edit, or rebuild it below" : "We'll build and publish your real website in about 60 seconds"}
            </div>
          </div>
        </div>
      </div>

      {/* Profile summary — what we already know */}
      <div style={{background:"#fff",border:`1px solid ${C.border}`,borderRadius:"12px",padding:"16px",marginBottom:"14px"}}>
        <div style={{fontWeight:700,fontSize:"0.82em",color:C.muted,textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:"10px"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:"6px"}}>
          {[
            ...
            ["🍽️", "Menu / Products", biz.menu],
          ].filter(([,, v]) => v).map(([icon, label, val]) => (
            <div key={label} style={{display:"flex",gap:"8px",fontSize:"0.82em"}}>
              <span style={{width:"20px",flexShrink:0}}>{icon}</span>
              <span style={{color:C.muted,width:"80px",flexShrink:0}}>{label}:</span>
              <span style={{color:C.text,fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{val}</span>
            </div>
          ))}
        </div>
        {missing.length > 0 && (
          <div style={{marginTop:"12px",padding:"10px 12px",background:C.amberLt,borderRadius:"8px",fontSize:"0.78em",color:"#78350F"}}>
            💡 Missing: {missing.join(", ")} — your website will still be built, but adding these in your profile settings makes it even better.
          </div>
        )}
      </div>

      {/* Optional extra fields */}
      {(phase === "ready" || phase === "gaps" || phase === "error") && (
        <div style={{background:"#fff",border:`1px solid ${C.border}`,borderRadius:"12px",padding:"16px",marginBottom:"14px"}}>
          <div style={{fontWeight:700,fontSize:"0.82em",color:C.muted,textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:"12px"}}>
            Optional — adds more detail to your site
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
            <div>
              <label style={{display:"block",fontWeight:600,fontSize:"0.82em",color:"#374151",marginBottom:"4px"}}>Opening hours</label>
              <input value={hours} onChange={e=>setHours(e.target.value)} placeholder="e.g. Mon–Fri 7am–5pm, Sat 8am–2pm" style={inputSt}/>
            </div>
            <div>
              <label style={{display:"block",fontWeight:600,fontSize:"0.82em",color:"#374151",marginBottom:"4px"}}>Street address (optional)</label>
              <input value={address} onChange={e=>setAddress(e.target.value)} placeholder="e.g. 42 Crown St, Wollongong" style={inputSt}/>
            </div>
            <div>
              <label style={{display:"block",fontWeight:600,fontSize:"0.82em",color:"#374151",marginBottom:"4px"}}>Anything special customers should know?</label>
              <textarea value={special} onChange={e=>setSpecial(e.target.value)} placeholder="Free parking, dog-friendly, 20 years in business, award-winning..." rows={2} style={{...inputSt,resize:"vertical"}}/>
            </div>
          </div>
        </div>
      )}
      ...
      {phase === "done" && liveUrl && (
        <div style={{marginBottom:"14px"}}>
          <div style={{background:C.greenLt,border:`1.5px solid #BBF7D0`,borderRadius:"12px",padding:"14px",marginBottom:"12px",textAlign:"center"}}>
            <div style={{fontWeight:800,color:C.green,marginBottom:"6px"}}>🎉 Your website is live!</div>
            <a href={liveUrl} target="_blank" rel="noopener noreferrer" style={{fontSize:"0.88em",fontWeight:700,color:C.green,wordBreak:"break-all"}}>
              🌐 {liveUrl}
            </a>
          </div>
          <div style={{background:"#fff",border:`1.5px solid ${C.border}`,borderRadius:"12px",overflow:"hidden",marginBottom:"10px"}}>
            <div style={{background:C.light,padding:"8px 12px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:"6px"}}>
              {["#FF5F56","#FFBD2E","#27C93F"].map(c=><div key={c} style={{width:"9px",height:"9px",borderRadius:"50%",background:c}}/>)}
              <div style={{flex:1,background:"#fff",borderRadius:"5px",padding:"3px 8px",fontSize:"0.7em",color:C.muted,border:`1px solid ${C.border}`,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{liveUrl}</div>
            </div>
            <iframe src={liveUrl} style={{width:"100%",height:"360px",border:"none",display:"block"}} title="Your live website" loading="lazy" key={liveUrl}/>
          </div>
          <div style={{display:"flex",gap:"8px",marginBottom:"10px"}}>
            <a href={liveUrl} target="_blank" rel="noopener noreferrer" style={{flex:1,padding:"12px",borderRadius:"9px",background:C.brand,color:"#fff",fontWeight:800,fontSize:"0.88em",textAlign:"center",textDecoration:"none"}}>
              🌐 Open Website
            </a>
            <button onClick={copyUrl} style={{flex:1,padding:"12px",borderRadius:"9px",border:`1.5px solid ${C.border}`,background:"#fff",color:copied?C.green:C.muted,fontWeight:600,fontSize:"0.88em",cursor:"pointer",fontFamily:"inherit"}}>
              {copied?"✓ Copied!":"📋 Copy Link"}
            </button>
          </div>
          <div style={{background:C.amberLt,border:`1px solid #FDE68A`,borderRadius:"9px",padding:"11px 13px",fontSize:"0.78em",color:"#78350F",lineHeight:1.6}}>
            💡 Want to change something? Use the <strong>Edit My Website</strong> button in this tab — describe what to change and it redeploys in 30 seconds.
          </div>
        </div>
      )}
      ...
      {/* Existing live URL */}
      {liveUrl && phase === "ready" && (
        <div style={{background:C.greenLt,border:`1.5px solid #BBF7D0`,borderRadius:"12px",padding:"14px",marginBottom:"14px"}}>
          <div style={{fontWeight:700,color:C.green,marginBottom:"6px",fontSize:"0.88em"}}>✅ Your website is already live</div>
          <a href={liveUrl} target="_blank" rel="noopener noreferrer" style={{fontSize:"0.85em",fontWeight:700,color:C.green,wordBreak:"break-all",display:"block",marginBottom:"10px"}}>
            🌐 {liveUrl}
          </a>
          <iframe src={liveUrl} style={{width:"100%",height:"280px",border:"none",borderRadius:"8px",display:"block",marginBottom:"10px"}} title="Your live website" loading="lazy"/>
          <div style={{fontSize:"0.75em",color:C.muted}}>Want a fresh version? Fill in the optional fields above and click Rebuild below.</div>
        </div>
      )}
      ...
      {phase !== "building" && (
        <button
          onClick={hasCritical ? buildSite : ()=>setPhase("gaps")}
          disabled={!hasCritical && phase !== "gaps"}
          style={{
        ...
        >
          {!hasCritical
            ? "⚠️ Update your profile with business name, suburb and description first"
            : liveUrl && phase==="ready"
            ? "🔄 Rebuild My Website →"
