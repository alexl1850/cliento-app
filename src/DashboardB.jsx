    ...
    try {
      const res = await fetch("/api/generate", {
        ...
        body: JSON.stringify({
          ...
          user:`Write a natural mention of ${member.name} that ${biz?.name||"this business"} could include in a blog post or their website.\n\n${biz?.name||"Our business"}: ${industry?.label||"local business"} in ${biz?.suburb||"Australia"} — ${biz?.description||"local services"}\n${member.name}: ${member.industry} in ${member.suburb} — ${member.desc}\nTheir website: ${member.website}\n\nWrite 3 different natural mention options (2-4 sentences each):\n\nOPTION A — for a blog post about local businesses\nOPTION B — for an "about us" or "partners" page\nOPTION C — for a social media post\n\nEach should feel genuinely written, explain why these businesses complement each other, give the reader a reason to visit, and include the website URL naturally. Sound like one local business owner recommending another they actually know.`,
          max_tokens: 800,
        })
      });
      const d = await res.json();
